...
var sampleSubject =  function() {
  ...
  var condition = flip(0.5) ? 'experimental' : 'control';

  //var accidentalVisitor = flip(0.3);

  // somehow, accidental visitors were assigned to red more often
  var accidentalVisitor = flip(condition == 'experimental' ? 0.45 : 0.1)
  var endorsement = condition == 'experimental'  ?
      discrete({ps: [1,2,3]}) : 
      discrete({ps: [3,2,1]})

  var time = accidentalVisitor? gaussian(2.5, 0.7) : gaussian(5, 0.75);

  var converted = accidentalVisitor ? false :
                  flip(condition == 'experimental' ? 0.9 : 0.5)//  ? 1: 0;
  return _.extend(location, 
            {time: Math.exp(time), browser: browser, condition: condition, converted: converted, endorsement: endorsement, accidentalVisitor: accidentalVisitor}
  )
}
repeat(200, sampleSubject)
"
...
observed_data <- rs %>%
  ...
  select(-accidentalVisitor)

write(
  jsonlite::toJSON(observed_data, pretty = T ),
"~/Documents/teaching/bdappl2018/assets/js/exptData.js")
```
